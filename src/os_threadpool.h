...
typedef struct os_threadpool {
	...
	pthread_cond_t aux_wait_cond; // condition for secondary threads
	int still_working; // 1 at start, set to 0 when all threads are waiting (there is no more work)
	int started_working; // 0 at start, set to 1 when added first task to list
	unsigned int aux_wait_cnt; // counts waiting secondary threads
} os_threadpool_t;
